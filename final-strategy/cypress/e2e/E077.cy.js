...
describe("E077 - Cargar correo vacio y contraseña vacia (a priori)", () => {
  const loginPage = new LoginPage(cy);
  const authenticationPage = new AuthenticationPage(cy);

  it("Cargar correo vacio y contraseña vacia (a priori)", async () => {
    //Ingresar a la página
    loginPage.visitPage();

    //Obtener data a priori
    const data = AUTH_DATA_POOL_A_PRIORI[0];
